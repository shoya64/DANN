...
def window_slice_MinMax_N(data, time_steps):
    data = np.transpose(data, (1, 0, 2)).reshape(-1, 310)
    mm = preprocessing.MinMaxScaler()
    data_min_max = mm.fit_transform(data)
    xs = []
    for i in range(int(data_min_max.shape[0]/time_steps) - 1):
        xs.append(data_min_max[i * time_steps: i * time_steps + time_steps])
    xs = np.concatenate(xs).reshape((len(xs), -1, 310))
    return xs
...
def main():
    ...
    #schedular = optim.lr_scheduler.StepLR(optimizer, step_size = 5, gamma = 0.9)
    ...
    for epoch in range(epochs_num):
        ...
        for i in range(int(training_size/batch_size)):
            ...
            #モデルに入力
            class_output, domain_output, out1 = model(data, alpha)
            ...
            #L1,L2正則化
            #L1_train = lamda1 * l1_penalty(out1)
            #L2_train = lamda2 * l2_penalty(out1)
            ...
            #テストデータのドメイン分類の出力を取得
            _, domain_t_output, _= model(data_t, alpha)
        ...
        for i in range(int(test_size/batch_size)):
            ...
            class_t_output ,domain_t_output , _ = model(data_t, alpha)
